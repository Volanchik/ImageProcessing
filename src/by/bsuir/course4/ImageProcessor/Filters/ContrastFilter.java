		...
		try {
			...
			for (int i = 0; i < height; ++i){
				for (int j = 0; j < width; ++j) {					
					...
					aggregationBlue += 10*getBlueComponent(getPixelValue(i, j));
					aggregationGreen += 10*getGreenComponent(getPixelValue(i, j));
					aggregationRed += 10*getRedComponent(getPixelValue(i, j));
					...
					aggregationGreen = 0;
					aggregationRed = 0;
					secondImagePixels[i*width + j] = pixel;
					
				}
				}


			File resultFile = new File(ROOT_PATH + "\\" + "ContrastFilter.png");
